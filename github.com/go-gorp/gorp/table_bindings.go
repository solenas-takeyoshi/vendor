...
func (t *TableMap) bindInsert(elem reflect.Value) (bindInstance, error) {
	...
	plan.once.Do(func() {
		...
		s := bytes.Buffer{}
		s2 := bytes.Buffer{}
		// 2018/07/31 Edit by N.Takeyoshi
...
func (t *TableMap) bindUpdate(elem reflect.Value, colFilter ColumnFilter) (bindInstance, error) {
	...
	plan.once.Do(func() {
		s := bytes.Buffer{}
		// 2018/08/01 Edit by N.Takeyoshi
		// s.WriteString(fmt.Sprintf("update %s set ", t.dbmap.Dialect.QuotedTableForQuery(t.SchemaName, t.TableName)))
		s.WriteString(fmt.Sprintf("update %s set ", t.TableName))
		x := 0
	...
	plan.once.Do(func() {
		...
		for _, col := range t.Columns {
			if !col.Transient {
				...
				// 2018/07/18 Edit by N.Takeyoshi
				// s.WriteString(t.dbmap.Dialect.QuoteField(col.ColumnName))
				s.WriteString(col.ColumnName)
				plan.argFields = append(plan.argFields, col.fieldName)
				x++
